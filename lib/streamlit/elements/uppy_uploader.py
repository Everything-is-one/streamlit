...
from streamlit.proto.UppyUploader_pb2 import UppyUploader as UppyUploaderProto
...
class UppyUploaderMixin:
    def uppy_uploader(
        ...
        """Display a uppy uploader widget.
        ...
        uppy_uploader_proto = UppyUploaderProto()
        uppy_uploader_proto.label = label
        uppy_uploader_proto.type[:] = type if type is not None else []
        uppy_uploader_proto.max_upload_size_mb = config.get_option(
            "server.maxUploadSize"
        )
        uppy_uploader_proto.multiple_files = accept_multiple_files
        uppy_uploader_proto.form_id = current_form_id(self.dg)
        if help is not None:
            uppy_uploader_proto.help = dedent(help)
        ...
        if ctx is not None and len(uploaded_file_info) != 0:
            ...
            ctx.uploaded_file_mgr.remove_orphaned_files(
                ...
                widget_id=uppy_uploader_proto.id,
                newest_file_id=newest_file_id,
                active_file_ids=active_file_ids,
            )

        self.dg._enqueue("uppy_uploader", uppy_uploader_proto)
